...
export const ListPage: React.FC = () => {
  ...
  const addByIndex = async () => {
    if (inputValue && inputIndex && inputIndex < listContainer.length) {
      ...
      listContainer.forEach((item: ICircle) => item.state = ElementStates.Default);
      await sleep(SHORT_DELAY_IN_MS);
      setListContainer([...listContainer]);
      setInputValue('');
      setInputIndex(null);
    ...
    <SolutionLayout title="Связный список">
      <div className={styles.listPage}>
        <div className={styles.listPage__controls}>
          <Input onChange={onChangeValue} value={inputValue} isLimitText maxLength={4} placeholder="Введите значение" />
          <Button style={{minWidth: 175}} isLoader={inProgress} disabled={!inputValue} onClick={onAddToHead} type="button" text="Добавить в head" />
          <Button style={{minWidth: 175}} isLoader={inProgress} disabled={!inputValue} onClick={onAddToTail} type="button" text="Добавить в tail" />
          <Button style={{minWidth: 175}} isLoader={inProgress} disabled={isListEmpty} onClick={onRemoveFromHead} type="button" text="Удалить из head" />
          <Button style={{minWidth: 175}} isLoader={inProgress} disabled={isListEmpty} onClick={onRemoveFromTail} type="button" text="Удалить из tail" />
        </div>
        <div className={styles.listPage__controls}>
          <Input onChange={onChangeIndex} value={inputIndex || ''} placeholder="Введите индекс" />
          <Button style={{minWidth: 362}} isLoader={inProgress} type="button" onClick={addByIndex} text="Добавить по индексу" />
          <Button style={{minWidth: 362}} isLoader={inProgress} type="button" onClick={removeByIndex} text="Удалить по индексу" />
        </div>
